...
/***/ function(module, exports, __webpack_require__) {
	...
	var _loginDialogJsx = __webpack_require__(3);
	
	var _loginDialogJsx2 = _interopRequireDefault(_loginDialogJsx);
	
	var _jquery = __webpack_require__(5);
	
	var _jquery2 = _interopRequireDefault(_jquery);
	
	var _getSetReff = __webpack_require__(12);
	
	var _getSetReff2 = _interopRequireDefault(_getSetReff);
	
	var $header = undefined;
	var updateStickyHeader = function updateStickyHeader() {
		if ((0, _jquery2['default'])(window).scrollTop() || window.innerWidth < 768 && (0, _jquery2['default'])('#hamburger-menu').hasClass('open')) {
	...
	function stickyHeader(header, noTransparency) {
		$header = (0, _jquery2['default'])(header);
		if (!noTransparency) {
			(0, _jquery2['default'])(window).on('scroll', updateStickyHeader);
			updateStickyHeader();
		} else {
			$header.removeClass('transparent');
		}
		...
		$header.find('.login').on('click', function () {
			_loginDialogJsx2['default'].show();
		});
	}
	
	function hamburgerMenu(header) {
		var $button = (0, _jquery2['default'])(header).find('#hamburger-menu-button');
		var $container = (0, _jquery2['default'])(header).find('#hamburger-menu');
		$button.on('click', function () {
			if ($button.hasClass('open')) {
				...
				setTimeout(function () {
					return $container.removeClass('closed');
				}, 200);
			} else {
		...
		$container.find('.menu-item-has-children > a').on('click', function (ev) {
			var $list = (0, _jquery2['default'])(ev.target).parent();
	...
	function initHeader(header, noTransparency) {
		stickyHeader(header, noTransparency);
		hamburgerMenu(header);
	
		_getSetReff2['default'].getSetReff(); // store traffic sources in referer chain cookie
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _apiJs = __webpack_require__(4);
	
	var _jsxdom = __webpack_require__(13);
	
	var _jsxdom2 = _interopRequireDefault(_jsxdom);
	...
	var _h5f = __webpack_require__(18);
	...
	var LoginDialog = (function () {
		function LoginDialog() {
			...
			this.dom = (0, _jsxdom2['default'])(
				...
				(0, _jsxdom2['default'])(
					...
					(0, _jsxdom2['default'])(
						'div',
						null,
						(0, _jsxdom2['default'])('input', { type: 'email', name: 'username',
					...
					(0, _jsxdom2['default'])(
						'div',
						null,
						(0, _jsxdom2['default'])('input', { type: 'password', name: 'password',
					...
					(0, _jsxdom2['default'])(
						'div',
						null,
						(0, _jsxdom2['default'])('input', { type: 'submit', value: 'Log in' })
					)
				),
				(0, _jsxdom2['default'])(
					'div',
					...
					(0, _jsxdom2['default'])(
						'p',
						{ 'class': 'error' },
						'Your username and/or password doesn\'t match our records.'
					),
					(0, _jsxdom2['default'])(
						'p',
						{ 'class': 'success' },
			...
			this.$form = (0, _jquery2['default'])(this.dom).find('form');
	
			_h5f2['default'].setup(this.$form[0]);
			...
			this.$form.on('submit', function (ev) {
				...
				if (!_this.$form[0].checkValidity()) {
					...
					// since H5F already puts a .required/.error class
					// to the inputs, we just have to move them up a level
					_this.$form.children('div').removeClass('invalid');
	
					_this.$form.find('.required, .error').each(function (i, e) {
						return (0, _jquery2['default'])(e).parent().addClass('invalid');
					});
				} else {
					_this.$dom.removeClass('error');
					(0, _apiJs.postLogin)(_this.$form.serialize()).then(function () {
						_this.$dom.addClass('success');
						(0, _apiJs.redirectToApp)();
					})['catch'](function (error) {
						_this.$dom.addClass('error');
					});
				}
			});
		}
		...
		_createClass(LoginDialog, [{
			key: 'show',
			value: function show() {
				_jquery2['default'].magnificPopup.open({ items: { src: this.dom },
					type: 'inline'
				});
			}
		}]);
	...
	exports['default'] = new LoginDialog();
...
/***/ function(module, exports, __webpack_require__) {
	...
	var APP_BASE = "https://staging.cargomatic.com";
	var API_BASE = APP_BASE + '/api/1';
	var COMMAND = "https://command.cargomatic.com";

	function postUser(formdata, isCarrier) {
		var endpoint = isCarrier ? 'carriers' : 'shippers';
		return new Promise(function (resolve, reject) {
			_jquery2['default'].post(API_BASE + '/' + endpoint, formdata).then(function (response) {
				if (!response.success) {
					switch (response.message.code) {
						case 11000:
							reject(new Error('existingEmail'));
							break;
						default:
	...
	function postLogin(formdata) {
		return new Promise(function (resolve, reject) {
			_jquery2['default'].post(APP_BASE + '/users/login', formdata).then(function (response) {
				if (!response.success) {
					...
					xdLocalStorage.init({
						// This is a problem
						iframeUrl: APP_BASE + '/xdls',
						initCallback: function initCallback() {
							Promise.all([xdLocalStorage.setItem('cargomatic-token', JSON.stringify(response.token)), xdLocalStorage.setItem('cargomatic-user', JSON.stringify(response.user))]).then(resolve());
						}
					});
				}
			}, function (error) {
	...
	function postSFLead(formdata, isCarrier) {
		formdata = _queryString2['default'].parse(formdata);
	
		// get the last utm-type campaign
		var reff = _getSetReff2['default'].parse().filter(function (e) {
			return e.type === "utm" || e.type === "organic";
		});
		var defaultReff = { medium: "not-set",
			source: "not-set",
			campaign: "not-set",
			content: "not-set",
			term: "not-set"
		...
		reff = reff[reff.length - 1] || defaultReff;
	
		if (reff.type === "organic") {
			var source = undefined,
			    medium = undefined;
	
			// for organic search, we reuse the source and medium fields
			// based on Devon Heck's instructions.
			if (reff.subtype === "search") {
				source = 'SEO';
			} else if (reff.subtype === "social") {
				source = 'SMO';
			}
	
			// fortunately only google's abbreavation is different than in the lib.
			if (reff.service === 'gg') {
				medium = 'GA';
			} else if (reff.service === 'bn') {
				medium = 'BA';
			} else {
				medium = reff.service.toUpperCase();
			}
		...
		return new Promise(function (resolve, reject) {
			...
			_jquery2['default'].post('/salesforce/servlet/servlet.WebToLead?encoding=UTF-8', { oid: '00Di0000000fLr2',
				retURL: 'http://example.com',
				...
				recordType: isCarrier ? '012i0000001IZJg' : '012i0000001IZJb',
				lead_source: 'Cargomatic Brand Site',
				Medium__c: reff.medium,
				Source__c: reff.source,
				Campaign__c: reff.campaign,
				Content__c: reff.content,
				Term__c: reff.term
			}).then(resolve, reject);
		});
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_RESULT__;/* WEBPACK VAR INJECTION */(function(module) {(function(global) {
	...
	} else if (__webpack_require__(15) && __webpack_require__(16)) {
	  !(__WEBPACK_AMD_DEFINE_FACTORY__ = (JSXDOM), __WEBPACK_AMD_DEFINE_RESULT__ = (typeof __WEBPACK_AMD_DEFINE_FACTORY__ === 'function' ? (__WEBPACK_AMD_DEFINE_FACTORY__.call(exports, __webpack_require__, exports, module)) : __WEBPACK_AMD_DEFINE_FACTORY__), __WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__));
	} else {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_RESULT__;/*! H5F
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _headerJs = __webpack_require__(2);
	
	var _headerJs2 = _interopRequireDefault(_headerJs);
	
	var _dialogJsx = __webpack_require__(29);
	...
	(0, _jquery2['default'])(function () {
		(0, _headerJs2['default'])((0, _jquery2['default'])('header'));
	
		var $typeSelect = (0, _jquery2['default'])('#select_type');
		var $termsCheckbox = (0, _jquery2['default'])('#terms');
		var $form = (0, _jquery2['default'])('#form');
		var isCarrier = undefined;
		var isShipper = undefined;
	
		var precheck = function precheck() {
			if (document.location.hash === '#carrier') {
				$typeSelect.find('[value=carrier]').prop('checked', true);
				$typeSelect.find('[value=shipper]').prop('checked', false);
			}
			
			if (document.location.hash === '#shipper') {
				$typeSelect.find('[value=shipper]').prop('checked', true);
				$typeSelect.find('[value=carrier]').prop('checked', false);
				
			}	
			
			if (!(document.location.hash === '#shipper') && !(document.location.hash === '#carrier')) {
				$typeSelect.find('[value=shipper]').prop('checked', false);
				$typeSelect.find('[value=carrier]').prop('checked', false);
		...
		var updateType = function updateType() {
			isCarrier = $typeSelect.find('[value=carrier]').prop('checked');
			isShipper = $typeSelect.find('[value=shipper]').prop('checked');
			$typeSelect.children('div').removeClass('checked');
	
			if (isCarrier) {
				$typeSelect.find('.carrier-container').addClass('checked');

				(0, _jquery2['default'])('#dot_number').parent().show();
				(0, _jquery2['default'])('#dot_number').attr('required', '');

				(0, _jquery2['default'])('#select_service').show();
				(0, _jquery2['default'])('#select_service input').attr('required', '');

				(0, _jquery2['default'])('#select_fleet_size').show();
				(0, _jquery2['default'])('#select_fleet_size input').attr('required', '');

				(0, _jquery2['default'])('#select_shipment').hide();
				(0, _jquery2['default'])('#select_shipment input').removeAttr('required');

				(0, _jquery2['default'])('#select_ship_volume').hide();
				(0, _jquery2['default'])('#select_ship_volume input').removeAttr('required');

			} else if (isShipper) {
				$typeSelect.find('.shipper-container').addClass('checked');
				
				(0, _jquery2['default'])('#dot_number').parent().hide();
				(0, _jquery2['default'])('#dot_number').removeAttr('required');

				(0, _jquery2['default'])('#select_service').hide();
				(0, _jquery2['default'])('#select_service input').removeAttr('required');

				(0, _jquery2['default'])('#select_fleet_size').hide();
				(0, _jquery2['default'])('#select_fleet_size input').removeAttr('required');

				(0, _jquery2['default'])('#select_shipment').show();
				(0, _jquery2['default'])('#select_shipment input').attr('required', '');

				(0, _jquery2['default'])('#select_ship_volume').show();
				(0, _jquery2['default'])('#select_ship_volume input').attr('required', '');
			} else {
				(0, _jquery2['default'])('#dot_number').parent().hide();
				(0, _jquery2['default'])('#dot_number').removeAttr('required');
				(0, _jquery2['default'])('#select_service').hide();
		...
		var updateTerms = function updateTerms() {
			return $termsCheckbox[0].checked ? $termsCheckbox.parent().addClass('checked') : $termsCheckbox.parent().removeClass('checked');
		};
	
		$typeSelect.on('click', updateType);
		$termsCheckbox.on('click', updateTerms);
	
		precheck();
		updateType();
		updateTerms();
	
		_h5f2['default'].setup($form);
	
		$form.find('input, textarea').on('invalid', function (ev) {
			if (HTMLInputElement.prototype.setCustomValidity) {
				ev.target.setCustomValidity((0, _jquery2['default'])(ev.target).closest('.form_field').find('.validation-error').text());
	
				dataLayer.push({ event: 'form-invalid',
					formType: 'signup',
					inputName: (0, _jquery2['default'])(ev.target).attr('name')
				});
			}
		});
	
		$form.find('input, textarea').on('input', function (ev) {
			if (HTMLInputElement.prototype.setCustomValidity) {
		...
		$form.on('submit', function (ev) {
			...
			//PARDOT HANDLER
			$terms.closest('.form_field').removeClass('invalid');
			if (!$form[0].checkValidity()) {
				// show validation error messages
				// since H5F already puts a .required/.error class
				// to the inputs, we just have to move them up a level
				$form.find('.form_field').removeClass('invalid');
	
				$form.find('.required, .error').each(function (i, e) {
					return (0, _jquery2['default'])(e).closest('.form_field').addClass('invalid');
				}).each(function (i, e) {
					return dataLayer.push({ event: 'form-invalid',
						formType: 'signup',
						inputName: e.name
					});
				});
			} else if (!$terms.is(':checked')) {
				$terms.closest('.form_field').addClass('invalid');
				dataLayer.push({ event: 'form-invalid',
					formType: 'signup',
					inputName: 'terms'
				});
			} else {
				(0, _apiJs.postUser)($form.serialize(), isCarrier)/*.then(function (response) {
					return (0, _apiJs.postLogin)({ username: $form.find('#email').val(),
						password: $form.find('#password').val()
					});
				})*/.then(function (response) {
					return (0, _apiJs.redirectToApp)();
					/*
					var accountType = isCarrier ? 'carrier' : 'shipper';
					dataLayer.push({ event: 'form-success',
						formType: 'signup',
						accountType: accountType
					});
					(0, _apiJs.postSFLead)('' + $form.serialize(), isCarrier)['catch'](function () {
						dataLayer.push({ event: 'form-lead-submit-failed',
							formType: 'signup'
						});
						console.error('signup.js: failed to submit lead');
					}).then(function () {
						// wait a second so every tag has enough time
						// to fire
						setTimeout(function () {
							return (0, _apiJs.redirectToApp)();
						}, 1000);
					...
					(0, _dialogJsx2['default'])('error', error.message);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var messages = { existingEmail: (0, _jsxdom2['default'])(
			'div="cm-error-msg"',
			null,
			'Sorry, but somebody has already registered with this email address. Did you',
			(0, _jsxdom2['default'])(
				'a',
				{ href: '#reset-password' },
		...
		unknown: (0, _jsxdom2['default'])(
			'div',
			null,
			'Sorry, but somebody has already registered with this email address or your fields are incomplete. Please try again, or',
			(0, _jsxdom2['default'])(
				'a',
				{ href: '/contact-us' },
				' contact us'
	...
	exports['default'] = function (type, message) {
		var dialog = (0, _jsxdom2['default'])('div', { 'class': type, id: 'dialog' });
	
		if (messages[message]) {
			dialog.appendChild(messages[message]);
		} else {
			console.error('dialog.js: dialog template not found');
			dialog.appendChild(messages.unknown);
		}
	
		(0, _jquery2['default'])(dialog).on('click', function () {
			return _jquery2['default'].magnificPopup.close();
		});
	
		_jquery2['default'].magnificPopup.open({ items: { src: dialog },
			type: 'inline'
		});
	};
